...
abstract class Kernel extends QMatrix {
    ...
    static double k_function(SvmNode[] x, SvmNode[] y, SvmParameter param) {
        switch (param.kernel_type) {
            ...
            case SvmParameter.PRECOMPUTED: // x: test (validation), y: SV
...
class Solver {
    ...
    void update_alpha_status(int i) {    //限定alpha的范围
        if (alpha[i] >= get_C(i))
    ...
    static class SolutionInfo {
        double obj;                //目标值
        double rho;                //为判决函数的偏置项b
        double upper_bound_p;    //第一个类的惩罚因子
        double upper_bound_n;    //第二个类的惩罚因子
    ...
    void Solve(int l,            //传入的样本的大小
               QMatrix Q,        //传入的样本特征集、标签集、训练参数
               double[] p_,        //已分配，初始化为{-1,...}
               byte[] y_,        //类向量，值为{+1,-1}
               double[] alpha_,    //待求的alpha值
               double Cp,        //第一个类的惩罚因子C_i
               double Cn,        //第二个类的惩罚因子C_j
               double eps,        //精度，在传入参数中设定，也称允许误差
               SolutionInfo si,    //待求的参数
               int shrinking    //是否收缩，默认为1
    ) {
        this.l = l;
        this.Q = Q;
        QD = Q.get_QD();
        ...
        //初始化alpha的状态位
        //根据惩罚因子调整alpha_i，调整规则：
        //	- alpha_i_status = 1    alpha_i >= C	 分离超平面与支持向量带之间的样本
        //	- alpha_i_status = 0    alpha_i <= 0	 远离支持向量的样本点
        //	- alpha_i_status = 2    0 < alpha_i < C	 支持向量样本
        {
        ...
        // 初始化梯度
        {
            ...
            for (int i = 0; i < l; i++)
                //如果alpha_i > 0，则进行调整，alpha_i<=0表示该样本已被正确分类且远离远离边界
                if (!is_lower_bound(i)) {
                    //Q = y_i * y_j * k(x_i, x_j)
                    float[] Q_i = Q.get_Q(i, l);
                    double alpha_i = alpha[i];
    ...
    int select_working_set(int[] working_set) {
        ...
        // 启发式寻找两个变量
        double Gmax = -INF;        //第一个变量的最大梯度
        double Gmax2 = -INF;    //第二个变量的最大梯度
        int Gmax_idx = -1;        //梯度最大的索引
        int Gmin_idx = -1;        //梯度最小的索引
        double obj_diff_min = INF;    //最小的差值
...
public class SVM {
    ...
    private static SvmPrintInterface svm_print_stdout = s -> {
        System.out.print(s);
        System.out.flush();
    };
    ...
    private static void solve_c_svc(SvmProblem prob,        //数据集
        ...
        if (Cp == Cn)
            SVM.info("nu = " + sum_alpha / (Cp * prob.size) + "\n");
...
//		}
    ...
    private static void solve_epsilon_svr(SvmProblem prob, SvmParameter param, double[] alpha,
        ...
        for (i = 0; i < l; i++) {
            alpha2[i] = 0;
            linear_term[i] = param.p - prob.label[i];
            y[i] = 1;

            alpha2[i + l] = 0;
            linear_term[i + l] = param.p + prob.label[i];
            y[i + l] = -1;
        }
    ...
    private static void solve_nu_svr(SvmProblem prob, SvmParameter param, double[] alpha, Solver.SolutionInfo si) {
        ...
        for (i = 0; i < l; i++) {
            ...
            linear_term[i] = -prob.label[i];
            y[i] = 1;

            linear_term[i + l] = prob.label[i];
            y[i + l] = -1;
        }
    ...
    static decision_function svm_train_one(SvmProblem prob,        //传入的训练集副本，默认是两个类的
                                           SvmParameter param,    //传入的训练参数
                                           double Cp,            //第一个类对应的惩罚因子C_i
                                           double Cn            //第二个类对应的惩罚因子C_j
    ) {
        //待求的alpha值
    ...
    private static void svm_binary_svc_probability(SvmProblem prob, SvmParameter param, double Cp, double Cn,
        ...
        for (i = 0; i < prob.size; i++)
            perm[i] = i;
        for (i = 0; i < prob.size; i++) {
            int j = i + rand.nextInt(prob.size - i);
            do {
                int tmp = perm[i];
                perm[i] = perm[j];
                perm[j] = tmp;
        ...
        for (i = 0; i < nr_fold; i++) {
            ...
            subprob.size = prob.size - (end - begin);
            subprob.X = new SvmNode[subprob.size][];
            subprob.label = new double[subprob.size];
        ...
        sigmoid_train(prob.size, dec_values, prob.label, probAB);
    }
    ...
    private static void svmGroupClasses(SvmProblem prob,
        ...
        int l = prob.size;
        int max_nr_class = 16;	//最多可以有16个类
        int nr_class = 0;		//类的个数
        int[] label = new int[max_nr_class];    //16个类各自是什么
        int[] count = new int[max_nr_class];    //每个类的统计数量
        int[] data_label = new int[l];          //转化标签，转换成0开头的

        for (int i = 0; i < l; i++) {
            int this_label = prob.label.get(i).intValue();
            int j;
            //对类的个数进行统计，最多统计16个类
...
//            SvmNode[][] x = new SvmNode[l][];
            ...
            for (i = 0; i < nr_class; i++)
                for (int j = i + 1; j < nr_class; j++) {
                    ...
                    SvmProblem subProb = new SvmProblem();
                    int si = start[i];    //第一个类开始的下标
                    int sj = start[j];    //第二个类开始的下标
                    int ci = count[i];    //第一个类的个数
                    int cj = count[j];    //第二个类的个数

                    subProb.size = ci + cj;                    //两个类的大小
                    subProb.X = new SvmNode[subProb.size][];    //两个类组成的特征集
                    subProb.label = new double[subProb.size];        //两个类组成的标签集
                    ...
                    for (int k = 0; k < cj; k++) {
                        ...
                        subProb.label[ci + k] = -1;
                    }

                    //判断是否计算SVC或SVR的概率估计，默认不计算
                    if (param.probability == 1) {
    ...
    public static String svmCheckParameter(SvmProblem prob, SvmParameter param) {
        ...
        if (kernel_type == SvmParameter.POLY
                || kernel_type == SvmParameter.RBF
                || kernel_type == SvmParameter.SIGMOID)
            if (param.gamma < 0)
                return "gamma < 0";
        ...
        //检查输入的d值，对多项式核有效
        if (kernel_type == SvmParameter.POLY && param.degree < 0)
        ...
        if (svm_type == SvmParameter.NU_SVC) {
            ...
            for (int i = 0; i < prob.size; i++) {
                Double thisLabel = prob.label.get(i);
                if (count.containsKey(thisLabel)) {
                    count.put(thisLabel, count.get(thisLabel) + 1);
                } else {
                    count.put(thisLabel, 1);
                }
            }
            //对类进行两两检查
            for (Map.Entry<Double, Integer> entry1 : count.entrySet()) {
                for (Map.Entry<Double, Integer> entry2 : count.entrySet()) {
                    if (entry1.equals(entry2)) {
                        if (param.nu * (entry1.getValue() + entry2.getValue()) / 2 > Math.min(entry1.getValue(), entry2.getValue())) {
